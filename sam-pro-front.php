  ...
  (adsbygoogle = window.adsbygoogle || []).push({
    ...
    enable_page_level_ads: true
		...
		private function isCrawler() {
			...
			if ( (int) $options['detectBots'] ) {
				switch ( $options['detectingMode'] ) {
					...
					case 'exact':
						if ( ! class_exists( 'SamProBrowser' ) ) {
							include_once( 'tools/sam-pro-browser.php' );
						}
		...
		public function headerScripts() {
			...
			if ( (int) $this->settings['useDFP'] ) {
				echo self::getDfpCodes( $this->settings['dfpMode'] );
			}
			if ( (int) $this->settings['enablePageLevelAds'] ) {
				echo self::getAdSenseCodes();
			}
			...
			wp_enqueue_script( 'samProLayout', apply_filters( 'sam_pro_front_layout_script', SAM_PRO_URL . 'js/sam.pro.layout.min.js' ), array(
				'jquery',
				'samProTracker'
			), '1.0.0.10' );
		...
		public function buildWhereClause() {
			...
			if ( is_home() || is_front_page() ) {
				...
				IF(sa.etypes, IF(sa.xtypes, TRUE, FALSE), TRUE) AND
				IF(sa.eauthors, IF(sa.xauthors, TRUE, FALSE), TRUE))";
			}
			if ( is_singular() ) {
				...
				if ( is_single() ) {
					...
					if ( ! empty( $tags ) ) {
						...
						foreach ( $tags as $val ) {
							$zone .= ( ( ! empty( $zone ) ) ? ',' : '' ) . "post_tag_{$val->slug}";
						}
					}
					if ( $rules['tags'] ) {
						if ( ! empty( $tags ) ) {
							...
							$sTags = "IF(sa.etags, IF(sa.xtags, TRUE, FALSE), TRUE)";
						}
					}
					...
					if ( $rules['taxes'] ) {
						if ( ! empty( $customTerms ) ) {
							...
							$sTaxes = "IF(sa.etax, IF(sa.xtax, TRUE, FALSE), TRUE)";
						}
					}
					...
					if ( ! empty( $sPost ) ) {
						$singular .= ( ( ( ! empty( $singular ) ) ? ' AND ' : '' ) . $sPost );
					}
					if ( ! empty( $cats ) ) {
						$singular .= ( ( ( ! empty( $singular ) ) ? ' AND ' : '' ) . $cats );
					}
					if ( ! empty( $sTags ) ) {
						$singular .= ( ( ( ! empty( $singular ) ) ? ' AND ' : '' ) . $sTags );
					}
					if ( ! empty( $sTaxes ) ) {
						$singular .= ( ( ( ! empty( $singular ) ) ? ' AND ' : '' ) . $sTaxes );
					}
					if ( ! empty( $sAuthor ) ) {
						$singular .= ( ( ( ! empty( $singular ) ) ? ' AND ' : '' ) . $sAuthor );
					}
				}
			...
			if ( is_archive() ) {
				...
				if ( is_tax() ) {
					...
					if ( ! is_wp_error( $cat ) ) {
						...
						if ( $rules['categories'] ) {
							$arc = "IF(sa.ecats, IF(sa.xcats, NOT FIND_IN_SET(\"{$cat->slug}\", sa.cats), FIND_IN_SET(\"{$cat->slug}\", sa.cats)), TRUE)";
						}
					}
				} elseif ( is_tag() ) {
			...
			$wcp    = apply_filters(
				'sam_pro_front_place_clause_logic',
				( ( ! empty( $singular ) ) ? ' AND (' . $singular . ')' : ( ( ! empty( $arc ) ) ? ' AND ' . $arc : ( ( ! empty( $home ) ? " AND {$home}" : '' ) ) ) )
			);
			$clause = apply_filters(
				'sam_pro_front_place_clause',
				"{$users} AND ((sa.ptype = 0) OR (sa.ptype = 1 AND (sa.ptypes+0 & {$viewPages}))){$wcp}{$schedule}",
				$rules
			);
		...
		public function doBlockShortcode( $atts ) {
			...
			$ad   = self::buildBlock( (int) $atts['id'], array(
				'id'   => $atts['id'],
				'name' => $atts['name']
			), $this->clause );
