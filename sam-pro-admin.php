	...
	class SamProAdmin extends SamProCore {
		...
		private function getWarningString( $mode = '' ) {
			...
			switch ( $mode ) {
				case 'cache':
					if ( SAM_PRO_W3TC ) {
						$text = __( 'Active W3 Total Cache plugin detected.', SAM_PRO_DOMAIN );
					} elseif ( SAM_PRO_WPSC ) {
						$text = __( 'Active WP Super Cache plugin detected.', SAM_PRO_DOMAIN );
					} else {
						$text = '';
					}
				...
				case 'forum':
					if ( SAM_PRO_BBP ) {
						$text = __( 'Active bbPress Forum plugin detected.', SAM_PRO_DOMAIN );
					} else {
						$text = '';
					}
  ...
  wum.meta_value > 1);";
			...
			array_push( $singleTaxes,
				array(
					'priority'  => 100,
					'slug'      => 'page_singular',
					'tax'       => __( 'Singular Pages', SAM_PRO_DOMAIN ),
					'name'      => $all,
					'term_slug' => 'page_singular'
				) );
			array_push( $singleTaxes,
				array(
					'priority'  => 90,
			...
			array_push( $singleTaxes,
				array(
					'priority'  => 3,
					'slug'      => 'page_attachment',
					'tax'       => __( 'Attachment', SAM_PRO_DOMAIN ),
					'name'      => $all,
					'term_slug' => 'page_attachment'
				) );
		...
		private function getIntervals() {
			...
			$out[0] = array(
				'start' => date( 'Y-m-01' ),
				'end'   => date( 'Y-m-t' ),
				'title' => __( 'This month', SAM_PRO_DOMAIN ) . " ({$mName}, " . date( 'Y' ) . ')'
			);
			for ( $i = 1; $i < $m; $i ++ ) {
				$cm       = $m - $i;
				$mName    = $months[ $cm - 1 ];
				...
				$out[ $i ] = array(
					...
					'title' => $mName . ', ' . $date->format( 'Y' )
				);
			}
			$out[20]  = array(
				'start' => date( 'Y-01-01' ),
				'end'   => date( 'Y-m-t' ),
				'title' => __( 'This year', SAM_PRO_DOMAIN ) . ' (' . date( 'Y' ) . ')'
			);
			$date     = new DateTime();
			...
			$out[21] = array(
				...
				'title' => __( 'Previous year', SAM_PRO_DOMAIN ) . ' (' . $date->format( 'Y' ) . ')'
			);
...
ORDER BY uu.owner;";
		...
		public function regAdminPage() {
			$menuPage              = add_menu_page( __( 'Ads', SAM_PRO_DOMAIN ), __( 'Ads', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-ads', array(
				&$this,
				'adsList'
			), 'dashicons-randomize' );
			$this->adsPage         = add_submenu_page( 'sam-pro-ads', __( 'Manage Ads', SAM_PRO_DOMAIN ), _x( 'Ads', 'Menu Bar', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-ads', array(
				&$this,
				'adsList'
			) );
			$this->adEditor        = add_submenu_page( 'sam-pro-ads', __( 'Ad Editor', SAM_PRO_DOMAIN ), __( 'Ad Editor', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-ad-editor', array(
				&$this,
				'adsEditor'
			) );
			$this->placesPage      = add_submenu_page( 'sam-pro-ads', __( 'Manage Ads Places', SAM_PRO_DOMAIN ), __( 'Places', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-places', array(
				&$this,
				'placesList'
			) );
			$this->placeEditor     = add_submenu_page( 'sam-pro-ads', __( 'Place Editor', SAM_PRO_DOMAIN ), __( 'Place Editor', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-place-editor', array(
				&$this,
				'placesEditor'
			) );
			$this->zonesPage       = add_submenu_page( 'sam-pro-ads', __( 'Manage Ads Zones', SAM_PRO_DOMAIN ), __( 'Zones', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-zones', array(
				&$this,
				'zonesList'
			) );
			$this->zoneEditor      = add_submenu_page( 'sam-pro-ads', __( 'Zone Editor', SAM_PRO_DOMAIN ), __( 'Zone Editor', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-zone-editor', array(
				&$this,
				'zonesEditor'
			) );
			$this->blocksPage      = add_submenu_page( 'sam-pro-ads', __( 'Manage Ads Blocks', SAM_PRO_DOMAIN ), __( 'Blocks', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-blocks', array(
				&$this,
				'blocksList'
			) );
			$this->blockEditor     = add_submenu_page( 'sam-pro-ads', __( 'Block Editor', SAM_PRO_DOMAIN ), __( 'Block Editor', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-block-editor', array(
				&$this,
				'blocksEditor'
			) );
			$this->advertisersList = add_submenu_page( 'sam-pro-ads', __( 'Advertisers', SAM_PRO_DOMAIN ), __( 'Advertisers', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-advertisers', array(
				&$this,
				'advList'
			) );
			$this->statsPage       = add_submenu_page( 'sam-pro-ads', __( 'Statistics', SAM_PRO_DOMAIN ), __( 'Statistics', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-statistics', array(
				&$this,
				'statistics'
			) );
			$this->toolsPage       = add_submenu_page( 'sam-pro-ads', __( 'Tools', SAM_PRO_DOMAIN ), __( 'Tools', SAM_PRO_DOMAIN ), 'manage_options', 'sam-pro-tools', array(
				&$this,
				'tools'
			) );
			do_action( 'sam_pro_admin_options_page_above_settings' );
			$this->settingsPage = add_submenu_page( 'sam-pro-ads', __( 'SAM Pro Settings', SAM_PRO_DOMAIN ), __( 'Settings', SAM_PRO_DOMAIN ), 'manage_options', 'sam-pro-settings', array(
				&$this,
				'adminPage'
			) );
			$this->errorLogPage = add_submenu_page( 'sam-pro-ads', __( 'Error Log', SAM_PRO_DOMAIN ), __( 'Error Log', SAM_PRO_DOMAIN ), SAM_PRO_ACCESS, 'sam-pro-error-log', array(
				&$this,
				'errorLog'
			) );
		...
		public function loadScripts( $hook ) {
			...
			if ( $hook == $this->settingsPage ) {
				...
				if ( $locale['changeLocale'] ) {
					...
					wp_enqueue_script( 'dt-locale', SAM_PRO_URL . "js/locales/ej.culture.ru-RU.min.js", array(
						'jquery',
						'globalize'
					) );
				}
				...
				wp_enqueue_script( 'sam-pro-ads-list', SAM_PRO_URL . 'js/sam.pro.ads.list.min.js', array(
					'jquery',
					'ej-all'
				) );
				...
				wp_enqueue_script( 'sam-pro-zones-list', SAM_PRO_URL . 'js/sam.pro.zones.list.min.js', array(
					'jquery',
					'ej-all'
				) );
				...
				wp_enqueue_script( 'sam-pro-blocks-list', SAM_PRO_URL . 'js/sam.pro.blocks.list.min.js', array(
					'jquery',
					'ej-all'
				) );
				...
				wp_enqueue_script( 'sam-pro-error-log', SAM_PRO_URL . 'js/sam.pro.error.log.min.js', array(
					'jquery',
					'ej-all'
				) );
				...
				wp_enqueue_script( 'sam-pro-block-editor', SAM_PRO_URL . 'js/sam.pro.block.editor.min.js', array(
					'jquery',
					'ej-all'
				), SAM_PRO_VERSION );
				...
				wp_enqueue_script( 'sam-pro-adverts', SAM_PRO_URL . 'js/sam.pro.advertisers.list.min.js', array(
					'jquery',
					'ej-all'
				), SAM_PRO_VERSION );
				...
				wp_enqueue_script( 'sam-pro-statistics', SAM_PRO_URL . 'js/sam.pro.statistics.min.js', array(
					'jquery',
					'ej-all'
				), SAM_PRO_VERSION );
		...
		public function addSettingsTab( $tab, $sections, $uri, $name, $class ) {
			...
			if ( ! empty( $sections ) ) {
				for ( $i = 0; $i < count( $sections ); $i ++ ) {
					...
					if ( $i === 0 ) {
						$this->settingsTabs[ $sections[ $i ][0] ] = array(
							'start_tab' => true,
							'uri'       => $uri,
							'name'      => $name,
							'class'     => $class
					...
					if ( $i === ( count( $sections ) - 1 ) ) {
						$this->settingsTabs[ $sections[ $i ][0] ]['finish_tab'] = true;
					}
				}
			}
		}
		...
		public function initSettings() {
			...
			self::addSettingsTab( 'general_tab', array(
				array(
					"sam_general_section",
					__( "General Settings", SAM_PRO_DOMAIN ),
					array( &$this, "drawGeneralSection" ),
					'sam-pro-settings'
				),
				array(
					"sam_ext_section",
					__( 'Extended Options', SAM_PRO_DOMAIN ),
					array( &$this, 'drawExtSection' ),
					'sam-pro-settings'
				),
				array(
					"sam_layout_section",
					__( "Admin Layout", SAM_PRO_DOMAIN ),
					array( &$this, "drawLayoutSection" ),
					'sam-pro-settings'
				),
				array(
					'sam_rules_section',
					__( 'Rules', SAM_PRO_DOMAIN ),
					array( &$this, 'drawRulesSection' ),
					'sam-pro-settings'
				),
				array(
					"sam_deactivate_section",
					__( "Plugin Deactivating", SAM_PRO_DOMAIN ),
					array( &$this, "drawDeactivateSection" ),
					'sam-pro-settings'
				)
			...
			self::addSettingsTab( 'embed_tab', array(
				array(
					"sam_single_section",
					__( "Content", SAM_PRO_DOMAIN ),
					array( &$this, "drawSingleSection" ),
					'sam-pro-settings'
				)
			...
			self::addSettingsTab( 'google_tab', array(
				array(
					"sam_dfp_section",
					__( "Google DFP Settings", SAM_PRO_DOMAIN ),
					array( &$this, "drawDFPSection" ),
					'sam-pro-settings'
				),
				array(
					'sam_adsense_section',
					__( 'Google AdSense', SAM_PRO_DOMAIN ),
					array( &$this, 'drawAdsenseSection' ),
					'sam-pro-settings'
				)
			...
			self::addSettingsTab( 'mailer_tab', array(
				array(
					'sam_mailer_section',
					__( 'Mailing System', SAM_PRO_DOMAIN ),
					array( &$this, 'drawMailerSection' ),
					'sam-pro-settings'
				),
				array(
					'sam_mailer_data_section',
					__( 'Mail Data', SAM_PRO_DOMAIN ),
					array( &$this, 'drawMailerDataSection' ),
					'sam-pro-settings'
				),
				array(
					'sam_mailer_content_section',
					__( 'Mail Content', SAM_PRO_DOMAIN ),
					array( &$this, 'drawMailerContentSection' ),
					'sam-pro-settings'
				),
				array(
					'sam_mailer_preview_section',
					__( 'Preview', SAM_PRO_DOMAIN ),
					array( &$this, 'drawPreviewSection' ),
					'sam-pro-settings'
				)
			), 'tabs-4', __( 'Mailer', SAM_PRO_DOMAIN ), 'icon-mail-alt' );
			self::addSettingsTab( 'tools_tab', array(
				array(
					"sam_statistic_section",
					__( "Statistics Settings", SAM_PRO_DOMAIN ),
					array( &$this, "drawStatisticsSection" ),
					'sam-pro-settings'
				),
				array(
					'sam_scavenger_section',
					__( 'Scavenge', SAM_PRO_DOMAIN ),
					array( &$this, 'drawScavengerSection' ),
					'sam-pro-settings'
				),
				array(
					"sam_html_section",
					__( "HTML Settings", SAM_PRO_DOMAIN ),
					array( &$this, "drawHtmlSection" ),
					"sam-pro-settings"
				)
			), 'tabs-6', __( 'Tools', SAM_PRO_DOMAIN ), 'icon-wrench' );
			do_action( 'sam_pro_settings_tabs' );

			add_settings_field( 'edition', 'Edition', array(
				&$this,
				'drawHiddenOption'
			), 'sam-pro-settings', 'sam_general_section', array( 'hidden' => true ) );
			add_settings_field( 'adCycle', __( "Views per Cycle", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_general_section', array( 'description' => __( 'Number of hits of one ad for a full cycle of rotation (maximal activity).', SAM_PRO_DOMAIN ) ) );
			add_settings_field( 'access', __( 'Minimum Level for the Menu Access', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawJSliderOption'
			), 'sam-pro-settings', 'sam_general_section', array(
				'description' => __( 'Who can use menu of plugin - Minimum User Level needed for access to menu of plugin. In any case only Super Admin and Administrator can use Settings Menu of SAM Plugin.', SAM_PRO_DOMAIN ),
				'options'     => array(
					'manage_network'    => __( 'Super Admin', SAM_PRO_DOMAIN ),
					'manage_options'    => __( 'Administrator', SAM_PRO_DOMAIN ),
					'edit_others_posts' => __( 'Editor', SAM_PRO_DOMAIN ),
					'publish_posts'     => __( 'Author', SAM_PRO_DOMAIN ),
					'edit_posts'        => __( 'Contributor', SAM_PRO_DOMAIN )
				),
				'values'      => array(
					'manage_network',
					'manage_options',
			...
			add_settings_field( 'adShow', __( "Ad Output Mode", SAM_PRO_DOMAIN ), array(
				...
				'description' => __( 'Standard (PHP) mode is more faster but is not compatible with caching plugins. If your blog use caching plugin (i.e WP Super Cache or W3 Total Cache) select "Caching Compatible (Javascript)" mode.', SAM_PRO_DOMAIN ),
				'options'     => array(
					'php' => __( 'Standard (PHP)', SAM_PRO_DOMAIN ),
					'js'  => __( 'Caching Compatible (Javascript)', SAM_PRO_DOMAIN )
				),
				'warning'     => 'cache'
			) );
			add_settings_field( 'adDisplay', __( "Display Ad Source in", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawRadioOption'
			), 'sam-pro-settings', 'sam_general_section', array(
				'description' => __( 'Target window (tab) for advertisement source.', SAM_PRO_DOMAIN ),
				'options'     => array(
					'blank' => __( 'New Window (Tab)', SAM_PRO_DOMAIN ),
					'self'  => __( 'Current Window (Tab)', SAM_PRO_DOMAIN )
				)
			) );
			do_action( 'sam_pro_settings_fields_general_section_before_plugins' );
			add_settings_field( 'bbpEnabled', __( 'Allow displaying ads on bbPress forum pages', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_general_section', array(
				'label_for' => 'bbpEnabled',
				'checkbox'  => true,
				'warning'   => 'forum',
				'enabled'   => ( ( defined( 'SAM_PRO_BBP' ) ) ? SAM_PRO_BBP : false )
			) );
			add_settings_field( 'wptouchEnabled', __( 'Allow displaying ads in the header of WPtouch mobile theme', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_general_section', array(
				'label_for' => 'wptouchEnabled',
				'checkbox'  => true,
				'warning'   => 'mobile',
			...
			add_settings_field( 'site_admin_url', __( 'Admin URL', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawHiddenOption'
			), 'sam-pro-settings', 'sam_general_section', array( 'hidden' => true ) );
			do_action( 'sam_pro_settings_fields_general_section' );

			add_settings_field( 'bpAdsId', __( "Ad Object before content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOptionEx'
			), 'sam-pro-settings', 'sam_single_section', array(
				'description' => '',
				/*'group' => array('slave' => null, 'master' => false, 'title' => __('Ad Object', SAM_PRO_DOMAIN).':')*/
			) );
			add_settings_field( 'beforePost', __( "Allow embedding of Ad Object before post/page content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'beforePost', 'checkbox' => true ) );
			add_settings_field( 'bpExcerpt', __( 'Allow embedding of Ad Object before post/page or post/page excerpt in the loop', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'bpExcerpt', 'checkbox' => true ) );
			add_settings_field( 'bbpBeforePost', __( "Allow embedding of Ad Object before bbPress Forum topic content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array(
				'label_for' => 'bbpBeforePost',
				'checkbox'  => true,
				'hide'      => self::hideBbpOptions()
			) );
			add_settings_field( 'bbpList', __( "Allow embedding of Ad Object into bbPress Forum forums/topics lists", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array(
				'label_for' => 'bbpList',
			...
			add_settings_field( 'bpUseCodes', __( "Allow using predefined Ad Object HTML tags (before and after codes)", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'bpUseCodes', 'checkbox' => true ) );
			add_settings_field( 'mpAdsId', __( "Ad Object within the content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOptionEx'
			), 'sam-pro-settings', 'sam_single_section', array(
				'description' => '',
			...
			add_settings_field( 'middlePost', __( "Allow embedding of Ad Object into the middle of post/page content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'middlePost', 'checkbox' => true ) );
			add_settings_field( 'bbpMiddlePost', __( "Allow embedding of Ad Object into the middle of bbPress Forum topic content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array(
				'label_for' => 'bbpMiddlePost',
				'checkbox'  => true,
				'hide'      => self::hideBbpOptions()
			) );
			add_settings_field( 'mpUseCodes', __( "Allow using predefined Ad Object HTML tags (before and after codes)", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'mpUseCodes', 'checkbox' => true ) );
			add_settings_field( 'apAdsId', __( "Ad Object after content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOptionEx'
			), 'sam-pro-settings', 'sam_single_section', array(
				'description' => '',
			...
			add_settings_field( 'afterPost', __( "Allow embedding of Ad Object after post/page content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'afterPost', 'checkbox' => true ) );
			add_settings_field( 'bbpAfterPost', __( "Allow embedding of Ad Object after bbPress Forum topic content", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array(
				'label_for' => 'bbpAfterPost',
				'checkbox'  => true,
				'hide'      => self::hideBbpOptions()
			) );
			add_settings_field( 'apUseCodes', __( "Allow using predefined Ad Object HTML tags (before and after codes)", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array( 'label_for' => 'apUseCodes', 'checkbox' => true ) );
			add_settings_field( 'wptAdsId', __( "Ad Object in the WPtouch header", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOptionEx'
			), 'sam-pro-settings', 'sam_single_section', array( 'description' => '', 'hide' => self::hideWptOptions() ) );
			add_settings_field( 'wptAd', __( "Allow embedding of Ad Object in the header of WPtouch mobile theme plugin", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_single_section', array(
				'label_for' => 'wptAd',
				'checkbox'  => true,
				'hide'      => self::hideWptOptions()
			) );
			do_action( 'sam_pro_settings_fields_single_section' );

			add_settings_field( 'useSWF', __( 'I use (plan to use) my own flash (SWF) banners. In other words, allow loading the script "SWFObject" on the pages of the blog.', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_ext_section', array( 'label_for' => 'useSWF', 'checkbox' => true ) );
			add_settings_field( 'errorlog', __( 'Turn on/off the error log.', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_ext_section', array( 'label_for' => 'errorlog', 'checkbox' => true ) );
			add_settings_field( 'errorlogFS', __( 'Turn on/off the error log for Front Side.', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_ext_section', array( 'label_for' => 'errorlogFS', 'checkbox' => true ) );
			do_action( 'sam_pro_settings_fields_ext_section' );

			add_settings_field( 'useDFP', __( "Allow using Google DoubleClick for Publishers (DFP) rotator codes", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_dfp_section', array( 'label_for' => 'useDFP', 'checkbox' => true ) );
			add_settings_field( 'dfpMode', __( 'Google DFP Mode', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawRadioOption'
			), 'sam-pro-settings', 'sam_dfp_section', array(
				'options'     => array(
					'gam' => __( 'GAM (Google Ad Manager)', SAM_PRO_DOMAIN ),
					'gpt' => __( 'GPT (Google Publisher Tag)', SAM_PRO_DOMAIN )
				),
				'description' => __( 'Select DFP Tags Mode.', SAM_PRO_DOMAIN )
			) );
			add_settings_field( 'dfpPub', __( "Google DFP Pub Code (GAM)", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_dfp_section', array(
				'description' => __( 'Your Google DFP Pub code. i.e:', SAM_PRO_DOMAIN ) . ' ca-pub-0000000000000000. ' . __( 'Only for GAM mode.', SAM_PRO_DOMAIN ),
				'width'       => '200px'
			) );
			add_settings_field( 'dfpNetworkCode', __( 'Google DFP Network Code (GPT)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_dfp_section', array(
				'description' => __( 'Network Code of Your DFP Ad Network. Only for GPT mode.', SAM_PRO_DOMAIN ),
				'width'       => '200px'
			) );
			do_action( 'sam_pro_settings_fields_dfp_section' );

			add_settings_field( 'adsensePub', __( 'Google AdSense Pub Code', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_adsense_section', array(
				'description' => __( 'Your Google AdSense Pub Code.' ),
				'placeholder' => 'ca-pub-0000000000000000',
				'width'       => '100%'
			) );
			add_settings_field( 'enablePageLevelAds', __( "Allow using Google AdSense page-level (<span id='anchor-help'>anchor/overlay</span> or/and <span id='vignette-help'>vignette</span>) ads on mobile devices", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_adsense_section', array(
				'label_for'   => 'enablePageLevelAds',
				'checkbox'    => true,
				'description' => __( 'Do not forget to adjust your Google AdSense settings on settings page (My Ads -> Page-level ads).', SAM_PRO_DOMAIN )
			) );

			add_settings_field( 'detectBots', __( "Allow Bots and Crawlers detection", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_statistic_section', array( 'label_for' => 'detectBots', 'checkbox' => true ) );
			add_settings_field( 'detectingMode', __( "Accuracy of Bots and Crawlers Detection", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawRadioOption'
			), 'sam-pro-settings', 'sam_statistic_section', array(
				'description' => __( "If bot is detected hits of ads won't be counted. Use with caution! More exact detection requires more server resources.", SAM_PRO_DOMAIN ),
				'options'     => array(
					'inexact' => __( 'Inexact detection', SAM_PRO_DOMAIN ),
					'exact'   => __( 'Exact detection', SAM_PRO_DOMAIN ),
					'more'    => __( 'More exact detection', SAM_PRO_DOMAIN ),
					'js'      => 'JS'
				)
			) );
			add_settings_field( 'stats', __( 'Allow to collect and to store statistical data', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_statistic_section', array( 'label_for' => 'stats', 'checkbox' => true ) );
			add_settings_field( 'keepStats', __( 'Keep Statistical Data', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOption'
			), 'sam-pro-settings', 'sam_statistic_section', array(
				'description' => __( 'Period of keeping statistical data (excluding current month).', SAM_PRO_DOMAIN ),
				'options'     => array(
					0  => __( 'All Time', SAM_PRO_DOMAIN ),
					1  => __( 'One Month', SAM_PRO_DOMAIN ),
					3  => __( 'Three Months', SAM_PRO_DOMAIN ),
					6  => __( 'Six Months', SAM_PRO_DOMAIN ),
					12 => __( 'One Year', SAM_PRO_DOMAIN )
				)
			) );
			add_settings_field( 'currency', __( "Display of Currency", SAM_PRO_DOMAIN ), array(
				...
				'description' => __( "Define display of currency. Auto - auto detection of currency from blog settings. USD, EUR - Forcing the display of currency to U.S. dollars or Euro.", SAM_PRO_DOMAIN ),
				'options'     => array(
					'auto' => __( 'Auto', SAM_PRO_DOMAIN ),
					'usd'  => __( 'USD', SAM_PRO_DOMAIN ),
					'euro' => __( 'EUR', SAM_PRO_DOMAIN )
				)
			) );
			do_action( 'sam_pro_settings_fields_statistic_section' );

			add_settings_field( 'moveExpired', __( 'Move expired ads to trash', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_scavenger_section', array( 'label_for' => 'moveExpired', 'checkbox' => true ) );
			add_settings_field( 'keepExpired', __( 'Permanently delete an expired ads', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawSelectOption'
			), 'sam-pro-settings', 'sam_scavenger_section', array(
				'options' => array(
					__( 'immediately', SAM_PRO_DOMAIN ),
					__( 'after 1 month', SAM_PRO_DOMAIN ),
					__( 'after 2 months', SAM_PRO_DOMAIN ),
					__( 'after 3 months', SAM_PRO_DOMAIN ),
					__( 'never', SAM_PRO_DOMAIN )
				)
			) );
			do_action( 'sam_pro_settings_fields_scavenger_section' );

			add_settings_field( 'samClasses', __( "Use Classes:", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawRadioOption'
			), 'sam-pro-settings', 'sam_html_section', array(
				'description' => __( 'Select method of the classes naming.' ),
				'options'     => array(
					'default' => __( 'Default', SAM_PRO_DOMAIN ),
					'custom'  => __( 'Custom', SAM_PRO_DOMAIN )
				)
			) );
			add_settings_field( 'containerClass', __( 'Custom SAM Container Class', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_html_section', array( 'width' => '200px' ) );
			add_settings_field( 'placeClass', __( 'Custom Place Class', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_html_section', array( 'width' => '200px' ) );
			add_settings_field( 'adClass', __( 'Custom Ad Class', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_html_section', array(
				'width'       => '200px',
				'description' => __( "Class names should consist of two parts (only letters and numbers) separated by symbol '-', ie 'bla-bla' or 'bye-bye'.", SAM_PRO_DOMAIN )
			) );
			do_action( 'sam_pro_settings_fields_html_section' );

			add_settings_field( 'itemsPerPage', __( "Items per Page", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_layout_section', array( 'description' => __( 'How many rows of the items list will be shown on one page of the data grid.', SAM_PRO_DOMAIN ) ) );
			do_action( 'sam_pro_settings_fields_layout_section' );

			add_settings_field( 'rule_id', __( 'Allow to use rule for posts/pages IDs', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_id', 'checkbox' => true ) );
			add_settings_field( 'rule_categories', __( 'Allow to use rule for categories', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_categories', 'checkbox' => true ) );
			add_settings_field( 'rule_tags', __( 'Allow to use rule for tags', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_tags', 'checkbox' => true ) );
			add_settings_field( 'rule_authors', __( 'Allow to use rule for authors', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_authors', 'checkbox' => true ) );
			add_settings_field( 'rule_taxes', __( 'Allow to use rule for Custom Taxonomies Terms', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_taxes', 'checkbox' => true ) );
			add_settings_field( 'rule_types', __( 'Allow to use rule for Custom Posts Types', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_types', 'checkbox' => true ) );
			add_settings_field( 'rule_schedule', __( 'Allow to use rule for scheduling', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_schedule', 'checkbox' => true ) );
			add_settings_field( 'rule_hits', __( 'Allow to use rule for restrictions by hits', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_hits', 'checkbox' => true ) );
			add_settings_field( 'rule_clicks', __( 'Allow to use rule for restrictions by clicks', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_rules_section', array( 'label_for' => 'rule_clicks', 'checkbox' => true ) );
			do_action( 'sam_pro_settings_fields_rules_section' );

			add_settings_field( 'deleteOptions', __( "Delete plugin options during deactivating plugin", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_deactivate_section', array( 'label_for' => 'deleteOptions', 'checkbox' => true ) );
			add_settings_field( 'deleteDB', __( "Delete database tables of plugin during deactivating plugin", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_deactivate_section', array( 'label_for' => 'deleteDB', 'checkbox' => true ) );
			add_settings_field( 'deleteFolder', __( "Delete custom images folder of plugin during deactivating plugin", SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_deactivate_section', array( 'label_for' => 'deleteFolder', 'checkbox' => true ) );
			do_action( 'sam_pro_settings_fields_deactivate_section' );

			add_settings_field( 'mailer', __( 'Allow SAM Mailing System to send statistical data to advertisers', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_section', array( 'label_for' => 'mailer', 'checkbox' => true ) );
			add_settings_field( 'mail_period', __( 'Periodicity of sending reports', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawRadioOption'
			), 'sam-pro-settings', 'sam_mailer_section', array(
				'options' => array(
					'monthly' => __( 'Monthly', SAM_PRO_DOMAIN ),
					'weekly'  => __( 'Weekly', SAM_PRO_DOMAIN )
				)
			) );
			do_action( 'sam_pro_settings_fields_mailer_section' );

			add_settings_field( 'mail_hits', __( 'Ad Hits (Number of shows of the advertisement)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_data_section', array( 'label_for' => 'mail_hits', 'checkbox' => true ) );
			add_settings_field( 'mail_clicks', __( 'Ad Clicks (Number of clicks on the advertisement)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_data_section', array( 'label_for' => 'mail_clicks', 'checkbox' => true ) );
			add_settings_field( 'mail_cpm', __( 'CPM (Cost per thousand impessions, calculated value)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_data_section', array( 'label_for' => 'mail_cpm', 'checkbox' => true ) );
			add_settings_field( 'mail_cpc', __( 'CPC (Cost per click, calculated value)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_data_section', array( 'label_for' => 'mail_cpc', 'checkbox' => true ) );
			add_settings_field( 'mail_ctr', __( 'CTR (Click through rate, calculated value)', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawCheckboxOption'
			), 'sam-pro-settings', 'sam_mailer_data_section', array( 'label_for' => 'mail_ctr', 'checkbox' => true ) );
			do_action( 'sam_pro_settings_fields_mailer_data_section' );

			add_settings_field( 'mail_subject', __( 'Mail Subject', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'Mail subject of sending email.', SAM_PRO_DOMAIN ),
				'width'       => '70%'
			) );
			add_settings_field( 'mail_greeting', __( 'Mail Greeting String', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'Greeting string of sending email.', SAM_PRO_DOMAIN ),
				'width'       => '70%'
			) );
			add_settings_field( 'mail_text_before', __( 'Mail Text before statistical data table', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextareaOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'Some text before statistical data table of sending email.', SAM_PRO_DOMAIN ),
				'height'      => '75px'
			) );
			add_settings_field( 'mail_text_after', __( 'Mail Text after statistical data table', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextareaOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'Some text after statistical data table of sending email.', SAM_PRO_DOMAIN ),
				'height'      => '75px'
			) );
			add_settings_field( 'mail_warning', __( 'Mail Warning 1', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextareaOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'This text will be placed at the end of sending email.', SAM_PRO_DOMAIN ),
				'height'      => '50px'
			) );
			add_settings_field( 'mail_message', __( 'Mail Warning 2', SAM_PRO_DOMAIN ), array(
				&$this,
				'drawTextareaOption'
			), 'sam-pro-settings', 'sam_mailer_content_section', array(
				'description' => __( 'This text will be placed at the very end of sending email.', SAM_PRO_DOMAIN ),
				'height'      => '50px'
			) );
			do_action( 'sam_pro_settings_fields_mailer_content_section' );

			add_settings_field( 'mail_preview', __( 'Mail Preview', SAM_PRO_DOMAIN ) . ':', array(
				&$this,
				'drawPreviewMail'
			), 'sam-pro-settings', 'sam_mailer_preview_section', array(
				'user' => array(
...
FROM {$pTable} sp WHERE sp.amode = 2;";
		...
		public function drawMailerSection() {
			...
			if ( (int) $options['mailer'] ) {
				...
				echo "<p>" . __( "Adjust parameters of Mailing System.", SAM_PRO_DOMAIN ) . "</p>";
			}
		}
      ...
      <input id="<?php echo $id; ?>"
		...
		public function drawCheckboxOption( $id, $args ) {
			...
			if ( isset( $args['enabled'] ) ) {
				$disabled = ( ( $args['enabled'] ) ? '' : 'disabled="disabled"' );
			}
			if ( isset( $args['hide'] ) ) {
