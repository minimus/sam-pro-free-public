...
if ( in_array( $action, $allowed_actions ) && samProIsValidURL($options) ) {
  ...
  IF(wsa.trash, \"<i class='icon-trash'></i>\", \"<i class='icon-leaf'></i>\")) AS trash";
			...
			else $out = array('count' => 0, 'result' => array());
			break;
  ...
  IF(wat.trash, \"<i class='icon-trash'></i>\", \"<i class='icon-leaf'></i>\")) AS stat";
			...
			$rows = $wpdb->get_results( $sql, ARRAY_A );
			foreach($rows as $key => $value) $rows[$key]['moderated'] = (boolean)$rows[$key]['moderated'];
		...
		case 'sam_pro_ajax_load_types':
			...
			foreach($post_types as $post_type) array_push($customs, array('title' => $post_type->labels->name, 'slug' => $post_type->name));
			$out = array(
				'count' => count($customs),
				'result' => $customs
			);
		...
		case 'sam_pro_ajax_clear_error_log':
			...
			$res = $wpdb->query($sql);
			if($res === false) $out = array('success' => false);
			else $out = array('success' => true, 'deleted' => $res);
			break;
...
FROM {$zrTable} szr WHERE szr.single = 0 AND szr.zid = {$zid}{$sqlWhere}{$sort}{$limit};";
		...
		case 'sam_pro_ajax_add_zone_rules':
			...
			else $out = array('success' => false, 'zid' => $zid,);
			break;
...
FROM {$zrTable} szr WHERE szr.single = 1 AND szr.zid = {$zid}{$sqlWhere}{$sort}{$limit};";
		...
		case 'sam_pro_ajax_load_regions':
			if(isset($params['co'])) {
				...
				foreach($aco as $iso) $valid &= (strlen($iso) == 2);
				if($valid) {
  ...
  GROUP BY mdate;";
				...
				if(!empty($sql)) $rows = $wpdb->get_results( $sql, ARRAY_A );
				else $rows = array();
				$out  = $rows;
			} else $out = array();
			break;
  ...
  IF(sa.trash, \"<i class='icon-trash'></i>\", \"<i class='icon-leaf'></i>\")) AS status";
			...
			$out = array('count' => count($rows), 'result' => $rows);
			break;
		default: $out = array('success' => false);
	}
