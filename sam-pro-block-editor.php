				...
				<div class="meta-box-sortables ui-sortable">
					<div id="headdiv-<?php echo $line.'-'.$column ?>" class="postbox" style="min-width: 175px;">
						...
						<div class="inside">
							...
							<label for="item-<?php echo $line.'-'.$column; ?>"><?php _e('Select Ad Object', SAM_PRO_DOMAIN); ?>:</label>
							<!--<input type='text' name='item-<?php echo $line.'-'.$column; ?>' class="block-item" id='item-<?php echo $line.'-'.$column; ?>'>-->
							<select class="block-item" id="item-<?php echo $line.'-'.$column; ?>" name="item-<?php echo $line.'-'.$column; ?>">
								<optgroup label="<?php _e('Places', SAM_PRO_DOMAIN); ?>">
									<?php foreach($places as $place) { ?>
									<option value="<?php echo $place['ival']; ?>" <?php selected($data[$line][$column], $place['ival']); ?>><?php echo $place['title'] ?></option>
									<?php } ?>
								</optgroup>
								<optgroup label="<?php _e('Ads', SAM_PRO_DOMAIN); ?>">
									<?php foreach($ads as $ad) { ?>
										<option value="<?php echo $ad['ival']; ?>" <?php selected($data[$line][$column], $ad['ival']); ?>><?php echo $ad['title'] ?></option>
									<?php } ?>
								</optgroup>
								<optgroup label="<?php _e('Zones', SAM_PRO_DOMAIN); ?>">
									<?php foreach($zones as $zone) { ?>
										<option value="<?php echo $zone['ival']; ?>" <?php selected($data[$line][$column], $zone['ival']); ?>><?php echo $zone['title'] ?></option>
									<?php } ?>
								</optgroup>
							</select>
											...
											<div id="minor-publishing">
												...
												<div id="misc-publishing-actions">
													...
													<div class="misc-pub-section">
														...
														<span id="place_size_info" class="post-status-display"><?php echo $row['b_rows'] . ' x ' . $row['b_columns']; ?></span>
													</div>
													<div class="misc-pub-section">
														<label for="trash_no">
									...
									<div class="postbox" id="block-style">
										...
										<div class="inside">
											<p>
												...
												<textarea id="b_style" name="b_style" placeholder="value_name: value;" style="width:100%;" rows="5"><?php echo htmlspecialchars(stripslashes($row['b_style'])); ?></textarea>
											</p>
											<p>
												<label for="i_style"><?php _e("Block's Item Style", SAM_PRO_DOMAIN); ?>:</label>
												<textarea id="i_style" name="i_style" placeholder="value_name: value;" style="width:100%;" rows="5"><?php echo htmlspecialchars(stripslashes($row['i_style'])); ?></textarea>
											</p>
